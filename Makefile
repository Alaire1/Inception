...
check_running:
        ...
        if [ "$$container_status" ]; then \
                printf "$(GREEN)Containers are already running. No need to relaunch.$(R>

        else \
                printf "$(YELLOW)Containers are not running. Checking data directories.>
                if [ ! "$(ls -A $(WORDPRESS_DATA))" ] || [ ! "$(ls -A $(MARIADB_DATA))">
                        printf "$(BLUE)Data directories are empty. Triggering a rebuild>
                        $(MAKE) create_dirs; \
                        $(MAKE) up_build; \
                else \
                        printf "$(BLUE)Data directories are not empty. Launching existi>
                        $(MAKE) up; \
                fi; \
        fi
